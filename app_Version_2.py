...
def render_futuristic_header():
    ...
    # --- INYECCIÓN DE SIDEBAR (SOLUCIÓN DEFINITIVA CON COMPONENTS) ---
    components.html("""
    <script>
        (function() {
            // Accedemos al documento padre (la aplicación Streamlit real)
            const doc = window.parent.document;
            
            // ID único para evitar duplicados
            const BTN_ID = 'sidebar-rescue-btn';
            
            // 1. LIMPIEZA PREVENTIVA
            // Limpiamos versiones anteriores para evitar fantasmas
            const legacyBtn = doc.getElementById('floating-toggle-btn');
            if (legacyBtn) legacyBtn.remove();
            
            const oldBtn = doc.getElementById(BTN_ID);
            if (oldBtn) oldBtn.remove();

            // 2. CREAR EL BOTÓN DESDE CERO
            const btn = doc.createElement('div');
            btn.id = BTN_ID;
            btn.innerHTML = '<i style="margin-bottom:5px; font-style:normal; font-size:16px;">☰</i> Filtros'; 
            
            // 3. ESTILOS "A PRUEBA DE BALAS" (Pegado al Body)
            ...
            // 4. INYECTAR EN EL BODY (Fuera de Streamlit)
            doc.body.appendChild(btn);

            // 5. LÓGICA DE CLIC
            btn.onclick = function(e) {
                e.stopPropagation(); // Evitar que Streamlit intercepte
                
                // Buscar el contenedor del sidebar original
                const marker = doc.getElementById('filter-sidebar-marker');
                
                if (marker) {
                    // Encontrar el bloque padre de Streamlit
                    ...
                    if (sidebar) {
                        ...
                        // MOVER EL BOTÓN: Si se abre, el botón viaja con el menú
                    ...
                    console.warn("⚠️ No se encontró el marcador #filter-sidebar-marker. ¿Se renderizó el sidebar?");
                }
            };
            
            console.log("✅ Botón de Filtros inyectado con éxito en el BODY.");
        })();
    </script>
    """, height=0, width=0)
