...
def render_premium_navbar():
    """
    Renders the unified top navigation bar with dropdowns.
    Replaces both option_menu and render_premium_submenu_v2.
    """
    ...
    # Build HTML for Navbar Items + Dropdowns
    nav_html = ""
    for module_name, subitems in SUBMENU_CONFIG.items():
        ...
        if subitems:
            ...
            for item in subitems:
                ...
                dropdown_html += f'<div class="dropdown-item {sub_active}" data-nav="{module_name}" data-sub="{item["key"]}"><span class="dropdown-label">{item["label"]}</span></div>'
            dropdown_html += '</div>'
        ...
        # Main item also gets data attributes
        nav_html += f'<div class="nav-item-wrapper {is_active}"><div class="nav-item" data-nav="{module_name}" data-sub="{default_sub}"><span class="nav-label">{module_name}</span>{arrow_html}</div>{dropdown_html}</div>'
...
.premium-navbar {{
    ...
    box-shadow: 0 8px 32px rgba(102, 126, 234, 0.4), 0 2px 8px rgba(0, 0, 0, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.2);
...
.dropdown-content {{
    display: none;
    position: absolute;
    top: 60px; /* Slight gap from header */
    left: 50%;
...
.dropdown-item:hover .dropdown-icon {{ opacity: 1; transform: scale(1.1); }}
...
<script>
// Attach logic to elements once they are in DOM
setTimeout(() => {{
    const items = document.querySelectorAll('.nav-item, .dropdown-item');
    items.forEach(el => {{
        el.addEventListener('click', function(e) {{
            ...
            console.log('Navigating to:', nav, sub);
            ...
            // Navigate using window.top to ensure full reload if inside iframe
...
# --- RENDERIZAR SUBMENÚ PREMIUM ---
# (Logic now handled internally by render_premium_navbar)
# selected_subtab IS ALREADY SET above.

# Validar cambio de módulo para resetear subtab si fuera necesario
# (Aunque render_premium_navbar ya maneja defaults, podemos forzar un refresh si cambia module sin subtab)
