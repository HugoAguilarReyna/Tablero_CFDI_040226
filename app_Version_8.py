...
SUBMENU_CONFIG = {
    "Dashboard": [
        {"label": "KPIs Operativos", "icon": "📊", "key": "kpis"},
        {"label": "Análisis Estructural", "icon": "🏗️", "key": "estructural"},
        {"label": "Tendencias", "icon": "📈", "key": "tendencias"}
    ],
    "Auditoría": [
        {"label": "Normativa", "icon": "📖", "key": "normativa"},
        {"label": "Documentos", "icon": "📋", "key": "documentos"},
        {"label": "Seguimiento", "icon": "✅", "key": "seguimiento"}
    ],
    "Riesgos": [
        {"label": "Detección de Anomalías", "icon": "⚠️", "key": "anomalias"},
        {"label": "Ranking de Riesgo", "icon": "📉", "key": "ranking"}
    ],
    "Compliance": [
        {"label": "Auditoría de Tasas", "icon": "🛡️", "key": "tasas"},
        {"label": "Integridad de Flujo", "icon": "💰", "key": "flujo"}
    ],
    "Configuración": [
        {"label": "General", "icon": "⚙️", "key": "general"},
        {"label": "Modelos AI", "icon": "🔮", "key": "ai"}
    ]
}
...
# --- RENDERIZAR SUBMENÚ PREMIUM ---
selected_subtab = None

if selected_module in SUBMENU_CONFIG:
    # Verificar si cambiamos de módulo
    if 'last_module' not in st.session_state or st.session_state.last_module != selected_module:
        st.query_params.clear()
        st.query_params["subtab"] = SUBMENU_CONFIG[selected_module][0]["key"]
...
def render_premium_submenu(module_name):
    """
    Renderiza un submenú premium de última generación con glassmorphism.
    """
    if module_name not in SUBMENU_CONFIG:
        return None
    
    items = SUBMENU_CONFIG[module_name]
    
    # Leer query params para obtener el subtab activo
    query_params = st.query_params
    active_key = query_params.get("subtab", items[0]["key"])
    
    # Validar que el key activo sea válido para este módulo
    valid_keys = [item["key"] for item in items]
    if active_key not in valid_keys:
        active_key = items[0]["key"]
    
    # Generar HTML de los items del submenú
    items_html = ""
    for idx, item in enumerate(items):
        is_active = "active" if item["key"] == active_key else ""
        
        items_html += f'''
        <div class="submenu-item {is_active}" 
             data-key="{item['key']}" 
             onclick="selectTab('{item['key']}', event)">
        ...
        if idx < len(items) - 1:
            items_html += '<span class="submenu-separator"></span>'
    ...
    <head>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .premium-submenu-container {{
            position: sticky; top: 0px; z-index: 9998; width: 100%;
            background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
            backdrop-filter: blur(20px) saturate(180%); -webkit-backdrop-filter: blur(20px) saturate(180%);
            box-shadow: 0 4px 20px rgba(102, 126, 234, 0.35), inset 0 1px 0 rgba(255, 255, 255, 0.15);
            border-bottom: 1px solid rgba(255, 255, 255, 0.2); border-top: 1px solid rgba(255, 255, 255, 0.05);
            padding: 14px 40px; display: flex; align-items: center; justify-content: center; gap: 8px;
            animation: slideDownPremium 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
        }}
        @keyframes slideDownPremium {{ from {{ opacity: 0; transform: translateY(-30px); filter: blur(10px); }} to {{ opacity: 1; transform: translateY(0); filter: blur(0); }} }}
        
        .submenu-item {{
            position: relative; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
            padding: 12px 24px; min-width: 140px; border-radius: 10px; border: 1.5px solid transparent;
            background: transparent; color: #E8E5FF; font-size: 13px; font-weight: 600; letter-spacing: 0.8px;
            text-transform: uppercase; cursor: pointer; user-select: none; transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1); overflow: hidden;
        }}
        .submenu-item:hover {{ background: rgba(255, 255, 255, 0.15); color: #FFFFFF; transform: translateY(-3px) scale(1.02); box-shadow: 0 8px 20px rgba(0,0,0,0.2); }}
        .submenu-item.active {{
            background: linear-gradient(135deg, rgba(255, 255, 255, 0.28), rgba(255, 255, 255, 0.22));
            border-color: rgba(255, 255, 255, 0.4); color: #FFFFFF; font-weight: 700;
            box-shadow: 0 6px 16px rgba(0,0,0,0.25), inset 0 2px 0 rgba(255,255,255,0.25); transform: translateY(0) scale(1);
        }}
        .submenu-item.active::after {{
            content: ''; position: absolute; bottom: -15px; left: 50%; transform: translateX(-50%);
            border-left: 10px solid transparent; border-right: 10px solid transparent; border-top: 10px solid rgba(255, 255, 255, 0.28);
            filter: drop-shadow(0 3px 6px rgba(0,0,0,0.2)); animation: arrowBounce 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
        }}
        @keyframes arrowBounce {{ 0% {{ bottom: -5px; opacity: 0; }} 100% {{ bottom: -15px; opacity: 1; }} }}
        
        .submenu-separator {{
            display: inline-block; width: 2px; height: 28px; margin: 0 12px;
            background: linear-gradient(180deg, transparent, rgba(255,255,255,0.4), transparent);
            animation: separatorGlow 3s ease-in-out infinite;
        }}
        @keyframes separatorGlow {{ 0%, 100% {{ opacity: 0.6; }} 50% {{ opacity: 1; }} }}
        
        @keyframes rippleExpand {{ from {{ transform: scale(0); opacity: 1; }} to {{ transform: scale(1); opacity: 0; }} }}
        </style>
    </head>
    <body>
        <div class="premium-submenu-container">{items_html}</div>
        <script>
        function createRippleEffect(event, element) {{
            ...
            ripple.style.cssText = `position: absolute; border-radius: 50%; background: radial-gradient(circle, rgba(255,255,255,0.6), transparent); width: ${{size}}px; height: ${{size}}px; left: ${{x}}px; top: ${{y}}px; pointer-events: none; animation: rippleExpand 0.7s cubic-bezier(0.4, 0, 0.2, 1);`;
            element.appendChild(ripple);
            setTimeout(() => ripple.remove(), 700);
        }}
        function selectTab(key, event) {{
            ...
            window.parent.history.pushState({{subtab: key}}, '', url);
            window.parent.dispatchEvent(new CustomEvent('subtabChanged', {{detail: {{key: key}}}}));
            setTimeout(() => {{ window.parent.location.href = url.href; }}, 100);
        }}
    ...
    """
    components.html(submenu_html, height=65, scrolling=False)
    return active_key
