...
def render_futuristic_header():
    ...
    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"])) {
        ...
        padding: 20px;
        padding-top: 80px; /* Más espacio superior para estética */
        overflow-y: auto;
        
        /* Glassmorphism adicional */
        backdrop-filter: blur(10px);
        -webkit-backdrop-filter: blur(10px);
        color: #FFFFFF !important; /* Force white text */
    }

    /* Efecto de overlay brillante superior */
    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"]))::before {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
    ...
    /* Force all text elements inside sidebar to be white/lavender */
    ...
    /* Dividers dentro del sidebar */
    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"])) hr {
        border-color: rgba(255, 255, 255, 0.2) !important;
        margin: 15px 0;
    }

    /* Widgets de Streamlit dentro del sidebar */
    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"])) [data-baseweb] {
        background-color: rgba(255, 255, 255, 0.1) !important;
        border-radius: 8px;
    }

    /* Scrollbar personalizada del sidebar */
    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"]))::-webkit-scrollbar {
        width: 8px;
    }

    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"]))::-webkit-scrollbar-track {
        background: rgba(0, 0, 0, 0.1);
        border-radius: 4px;
    }

    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"]))::-webkit-scrollbar-thumb {
        background: rgba(255, 255, 255, 0.3);
        border-radius: 4px;
    }

    div[data-testid="stVerticalBlock"]:has(#filter-sidebar-marker):not(:has([data-testid="stVerticalBlock"]))::-webkit-scrollbar-thumb:hover {
...
def plot_dark_histogram(df, x_col, y_col, color_col, title):
    fig = px.bar(df, x=x_col, y=y_col, color=color_col, 
                 color_discrete_sequence=['#667eea', '#764ba2', '#5a67d8', '#7c8ff0', '#a389f4'], 
                 title=title)
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, tickfont=dict(color='#E8E5FF')),
        yaxis=dict(showgrid=True, gridcolor='rgba(102, 126, 234, 0.1)', tickfont=dict(color='#E8E5FF')),
        ...
        selected_module = option_menu(
            ...
            styles={
                "container": {
                    ...
                    "background": "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",  # ← NUEVO GRADIENTE
                    "border-radius": "0px",
                    "border": "none",
                    "width": "100% !important",
                    "max-width": "none !important",
                    "box-shadow": "0 4px 20px rgba(102, 126, 234, 0.35)"  # ← Sombra ajustada
                },
                "icon": {
                    "color": "#FFFFFF", 
                    "font-size": "16px"
                ...
                "nav-link-selected": {
                    "background-color": "rgba(255, 255, 255, 0.2)",  # ← Overlay más visible
                    "color": "#FFFFFF",
                    "font-weight": "700",
                    "border-radius": "var(--radius-sm)",
    ...
    # --- SUB-TAB: STRUCTURAL ANALYSIS ---
        ...
        with c_cat1:
            if 'tipo' in df_filtered.columns:
                ...
                color_map = {'I': '#00E676', 'E': '#FF1744', 'N': '#FFFFFF', 'P': '#FFFFFF'}
                fig_tipo = px.bar(
    ...
    # --- SUB-TAB: TRENDS ---
        ...
        if not df_filtered.empty:
            ...
            fig_area.update_traces(
                ...
                marker=dict(size=8, color='#764ba2', line=dict(width=2, color='#ffffff')),
                text=df_w['total'].apply(lambda x: f'${x/1000:,.0f}k'),
        ...
        if selected_subtab == "anomalias":
            ...
            if all(col in df_filtered.columns for col in sankey_cols) and target_col:
                ...
                fig_sankey = go.Figure(data=[go.Sankey(
                    node = dict(pad = 20, thickness = 15, line = dict(color = "#667eea", width = 0.5), label = nodes, color = "#667eea"),
                    link = dict(source = links['emisor_nombre'].map(node_idx), target = links[target_col].map(node_idx), value = links['total'], color = 'rgba(102, 126, 234, 0.25)')
                )])
                fig_sankey.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family="Inter", size=10), height=500)
            ...
            with c2:
                ...
                fig_risk_dist = px.histogram(risk_summary, x='risk_score', nbins=10, title="Distribución de Riesgo", template="plotly_dark", color_discrete_sequence=['#764ba2'])
                fig_risk_dist.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family="Inter"))
            ...
            if not extreme_risk.empty:
                st.markdown(f"""
                <div style="background: rgba(255, 23, 68, 0.05); border: 2px solid #FF1744; padding: 15px; border-radius: 10px; margin-top: 20px; backdrop-filter: blur(5px);">
                    <h4 style="color: #FF1744; margin-top: 0; display: flex; align-items: center;">
                        <span style="margin-right: 10px;">⚠️</span> ALERTA DE CRITICALIDAD FISCAL
                    </h4>
                    <p style="color: #FFFFFF; font-size: 14px; margin-bottom: 0;">Se han detectado {len(extreme_risk)} emisores con un Score de Riesgo superior al 80%. 
     ...
     # --- TAB 2: FLOW INTEGRITY ---
         ...
         if 'metodo_pago' in df_filtered.columns:
             ...
             fig_gauge = go.Figure(go.Indicator(
                 ...
                 delta = {'reference': 100, 'increasing': {'color': "#00E676"}, 'decreasing': {'color': "#FF1744"}},
                 gauge = {
                     'axis': {'range': [None, 100], 'tickwidth': 1, 'tickcolor': "#E8E5FF"},
                     'bar': {'color': "#667eea"},
             ...
             fig_gauge.update_layout(paper_bgcolor = "rgba(0,0,0,0)", font = {'color': "white", 'family': "Inter, sans-serif"})
